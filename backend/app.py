...
def token_required(t):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            current_user = db.tbl_user.find_one(
    ...
    if users:
        if check_password_hash(users['password'], passwords):
            ...
            json_result = json_util.dumps(
                ({'token': token, 'username': user['name'], 'email': user['email'], 'password': user['password'], 'login': "users"}))
            return json_result
        else:
        ...
        if check_password_hash(user['password'], passwords):
            ...
            json_result = json_util.dumps(
                ({'token': token, 'username': user['name'], 'email': user['email'], 'password': user['password'], 'login': "user"}))
            return json_result
        else:
            return make_response({'error': "Passwords doesnt match!"})
    else:
...
def add_interst(current_user):
    ...
    uerdata = db.tbl_user.update_one(
        {'_id': ObjectId(current_user['_id'])}, {'$set': {'status': 1}})
...
def retriveresultsc(current_user):
    ...
    interestdata = db.tbl_interest.find_one(
        {'user_id': ObjectId(current_user['_id'])})

    # Check if interestdata is None
    if not interestdata:
        return jsonify({'message': 'No interest data found'}), 404
    lat1 = float(current_user['latitude'])
    lon1 = float(current_user['longtitude'])

    events_with_distances = []
    user_typeofintrest = 1  # Default value
    user_areaofinterest = 1  # Default value

    if interestdata['typeofintrest'] == "events":
        user_typeofintrest = 1
        eventtype = db.tbl_eventtype.find_one(
            {'Eventtype_name': interestdata['interst']})
        if eventtype is None:
            # Handle the case where 'eventtype' is not found
            return jsonify({'message': 'Event type not found'}), 404

        if interestdata['areaofinterest'] == "Outdoor":
            eventdata = db.tbl_events.find(
                {'area': "Outdoor", 'type': eventtype['_id']})
            user_areaofinterest = 1
        else:
            eventdata = db.tbl_events.find(
                {'area': "Indoor", 'type': eventtype['_id']})
            user_areaofinterest = 2

        # Now, you can calculate user_choices based on the user's preferences
        user_choices = [user_typeofintrest,
                        user_areaofinterest]

        for activity in eventdata:
            # Calculate the rating for each activity
            if activity["area"] == "Outdoor":
                type = 1
            elif activity["area"] == "Indoor":
                type = 2
            else:
                type = 3
            ...
            if interestdata['typeofintrest'] == "events":
                group = 1
            elif interestdata['typeofintrest'] == "activities":
                group = 2
            else:
                group = 3

            feature_list = [type, group]

            rating = dot(user_choices, feature_list) / (
                norm(user_choices) * norm(feature_list)
            )

            # Add the rating to the activity data
            activity['algoRating'] = rating
            recommended_activities.append(activity)

        for event in recommended_activities:
            lat2 = float(event['latitude'])
            lon2 = float(event['longtitude'])
            distance = (((lat1-lat2)**2)+((lon1-lon2)**2))**0.5
            event['distance'] = distance
            events_with_distances.append(event)
        events_with_distances = sorted(
            events_with_distances, key=lambda k: k.get("distance"), reverse=False)

        rec_serialized_data = json_util.dumps(events_with_distances)
        return rec_serialized_data

    else:
        user_typeofintrest = 1
        activitytype = db.tbl_activitytype.find_one(
            {'activitytype_name': interestdata['interst']})
        if activitytype is None:

            return jsonify({'message': 'activity type not found'}), 404
        if interestdata['areaofinterest'] == "Outdoor":
            user_areaofinterest = 1
            activitydata = db.tbl_activity.find(
                {'area': "Outdoor", 'type': activitytype['_id']})
            # print(type(activitydata))

        else:
            activitydata = db.tbl_activity.find(
                {'area': "Indoor", 'type': activitytype['_id']})
            user_areaofinterest = 1
            user_choices = [user_typeofintrest,
                            user_areaofinterest]
            ...
            #  lat2 = float(activity['latitude'])
            #  lon2 = float(activity['longtitude'])
            #  distance = (((lat1-lat2)**2)+((lon1-lon2)**2))**0.5
            #  activity['distance'] = distance
            #  events_with_distances.append(activity)
            # events_with_distances = sorted(
            #   events_with_distances, key=lambda k: k.get("distance"), reverse=False)
        # Sort the activities based on the algoRating
        # recommended_activities = sorted(recommended_activities, key=lambda k: k.get("algoRating"), reverse=True)

        rec_serialized_data = json_util.dumps(activitydata)
        return rec_serialized_data

        # Sort the activities based on the algoRating
        # recommended_activities = sorted(recommended_activities, key=lambda k: k.get("algoRating"), reverse=True)

        # rec_serialized_data = json_util.dumps(recommended_activities)
        # return rec_serialized_data
    # cursor = db.tbl_activitytype.find()
...
def vieweventwishlist(current_user):
    ...
    ids = []
    l = []
    userdata = db.tbl_eventwishlist.find(
        {'user_id': ObjectId(current_user['_id'])})
    for i in userdata:
        l.append(i['eventid'])
    print(len(l))
...
def viewactivitywishlist(current_user):
    ...
    ids = []
    l = []
    userdata = db.tbl_activitywishlist.find(
        {'user_id': ObjectId(current_user['_id'])})
    for i in userdata:
...
def deleteInterest(current_user):
    ...
    result = db.tbl_interest.delete_one(
        {'user_id': ObjectId(current_user['_id'])})
...
def deleteactivitywishlist(current_user, id):
    ...
    result = db.tbl_activitywishlist.delete_one(
        {'user_id': ObjectId(current_user['_id']), 'activityid': ObjectId(id)})
...
def deleteeventwishlist(current_user, id):
    ...
    result = db.tbl_eventwishlist.delete_one(
        {'user_id': ObjectId(current_user['_id']), 'eventid': ObjectId(id)})
...
def checkwishlist(current_user, id):
    ...
    print(id)
    result = db.tbl_eventwishlist.find_one(
        {'user_id': ObjectId(current_user['_id']), 'eventid': ObjectId(id)})

    result1 = db.tbl_activitywishlist.find_one(
        {'user_id': ObjectId(current_user['_id']), 'activityid': ObjectId(id)})
    if result:
